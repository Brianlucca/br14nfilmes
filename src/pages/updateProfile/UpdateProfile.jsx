...
const UpdateProfile = () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          const fetchedNickname =
            userData.nickname || `Usuário_${user.uid.substring(0, 5)}`;
  ...
  const checkNicknameExists = async (newNicknameToCheck) => {
    ...
    try {
      ...
      if (usersData) {
        for (let uidInUsers in usersData) {
          if (uidInUsers !== user.uid) {
            ...
            if (
              userNode.nickname &&
              userNode.nickname.toLowerCase() === lowerNickname
            ) {
              return true;
            }
          }
      ...
      setMessage({
        text: "Erro ao verificar o nickname!",
        type: "error",
        suggestions: [],
      });
      return true;
  ...
  const generateNicknameSuggestions = async (baseName) => {
    ...
    const base =
      baseName.replace(/[^a-zA-Z0-9]/g, "").substring(0, 10) ||
      generateRandomElement(thematicWords.general);
    ...
    while (suggestions.length < 3 && attempts < MAX_SUGGESTION_ATTEMPTS) {
      ...
      if (
        suggestion.length >= 3 &&
        !(await checkNicknameExists(suggestion)) &&
  ...
  useEffect(() => {
    const processDebouncedNickname = async () => {
      const nicknameToProcess = debouncedNickname;
      if (
        !user ||
        !nicknameToProcess.trim() ||
        nicknameToProcess.trim().toLowerCase() ===
          currentNickname.toLowerCase() ||
        nicknameToProcess.trim().length < 3
      ) {
        if (
          message.type !== "success" &&
          message.type !== "error" &&
          message.text !== "O novo nickname é igual ao atual." &&
          message.text !== "Verificando disponibilidade..."
        ) {
          setMessage({ text: "", type: "", suggestions: [] });
        }
        return;
      ...
      if (exists) {
        const newSuggestions = await generateNicknameSuggestions(
          nicknameToProcess.trim(),
        );
        setMessage({
          ...
          suggestions:
            newSuggestions.length > 0
              ? newSuggestions
              : ["Tente uma variação diferente ou adicione números."],
        });
      } else {
        setMessage({
          text: "Nickname disponível!",
          type: "success",
          suggestions: [],
        });
      }
  ...
  }, [debouncedNickname, currentNickname, user]);
  ...
  const handleUpdateProfile = async () => {
    ...
    if (
      ...
      message.type === "success" ||
      (message.type === "error" &&
        message.text === "Este nickname já está em uso!")
    ) {
      setMessage({ text: "", type: "", suggestions: [] });
    }
    ...
    if (!trimmedNickname) {
      setMessage({
        text: "O nickname não pode estar vazio!",
        type: "error",
        suggestions: [],
      });
    ...
    if (trimmedNickname.length < 3 || trimmedNickname.length > 20) {
      setMessage({
        text: "O nickname deve ter entre 3 e 20 caracteres.",
        type: "error",
        suggestions: [],
      });
      setLoadingSave(false);
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(trimmedNickname)) {
      setMessage({
        text: "Nickname inválido. Use apenas letras, números e os caracteres . _ -",
        type: "error",
        suggestions: [],
      });
      setLoadingSave(false);
    ...
    if (trimmedNickname.toLowerCase() === currentNickname.toLowerCase()) {
      setMessage({
        text: "O novo nickname é igual ao atual.",
        type: "info",
        suggestions: [],
      });
      setLoadingSave(false);
    ...
    if (nicknameChangeCount >= NICKNAME_CHANGE_LIMIT) {
      setMessage({
        text: `Você atingiu o limite de ${NICKNAME_CHANGE_LIMIT} trocas de nickname!`,
        type: "error",
        suggestions: [],
      });
      setLoadingSave(false);
    ...
    const nicknameHistoryRef = ref(
      database,
      `users/${user.uid}/nicknameHistory`,
    );
    ...
    try {
      ...
      if (
        currentNickname &&
        currentNickname !== `Usuário_${user.uid.substring(0, 5)}`
      ) {
      ...
      setMessage({
        text: "Nickname atualizado com sucesso!",
        type: "success",
        suggestions: [],
      });
      ...
      setMessage({
        text: "Erro ao atualizar o nickname. Tente novamente.",
        type: "error",
        suggestions: [],
      });
  ...
  const handleForceGenerateSuggestions = async () => {
    if (
      !nickname.trim() ||
      nickname.trim().toLowerCase() === currentNickname.toLowerCase()
    ) {
    ...
    if (newSuggestions.length > 0) {
      ...
      setMessage({
        text: "Não encontramos boas sugestões, tente algo diferente.",
        type: "info",
        suggestions: [],
      });
    }
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-black text-gray-300">
      <Sidebar />
      <div className="flex-grow md:ml-20 transition-all duration-300 ease-in-out flex flex-col md:mt-0 mt-14">
          ...
          <div className="w-full max-w-lg bg-[#1c1c1c] p-6 sm:p-8 shadow-2xl rounded-xl border border-gray-800/70">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400 mt-1">
                Seu nickname atual:{" "}
                <strong className="text-sky-400">{currentNickname}</strong>
              </p>
            </div>

            {message.text && (
              <div
                className={`mb-4 p-3 rounded-lg text-sm text-center flex flex-col items-center justify-center space-y-2
            ...
            <div className="space-y-6">
              <div>
                ...
                >
                  Novo Nickname (trocas restantes:{" "}
                  {Math.max(0, NICKNAME_CHANGE_LIMIT - nicknameChangeCount)})
                </label>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Entre 3-20 caracteres. Letras, números e ( . _ - ) são
                  permitidos.
                </p>
                <p className="text-xs text-gray-500 mt-0.5">
                  Máximo de {NICKNAME_CHANGE_LIMIT} trocas permitidas.
                </p>
              </div>
              <button
                ...
                disabled={
                  loadingSave ||
                  nicknameChangeCount >= NICKNAME_CHANGE_LIMIT ||
                  nickname.trim().toLowerCase() ===
                    currentNickname.toLowerCase() ||
                  !nickname.trim() ||
                  (message.type === "error" &&
            ...
            <p className="text-gray-400 text-xs text-center mt-6">
              Seu nickname é usado para identificar você em recomendações,
