...
const WatchSession = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-black text-gray-300">
      <Sidebar />
      <div className="flex-grow md:ml-20 relative mt-16 md:mt-0">
        {session.image && (
          <>
            <div
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-[#1c1c1c]/80 backdrop-blur-md p-5 sm:p-6 rounded-xl shadow-2xl border border-gray-800/60">
                  ...
                  {session.isSeries ? (
                    <div className="space-y-3 max-h-72 overflow-y-auto custom-scrollbar-dark pr-2">
                      {Array.from({ length: session.episodes || 0 }).map(
                        (_, index) => {
                          ...
                          return (
                            <div
                              key={episode}
                              className="flex items-center p-2.5 bg-gray-800/40 rounded-md hover:bg-gray-700/60 transition-colors"
                            >
                              <input
                                type="checkbox"
                                id={`episode-${episode}`}
                              ...
                              <label
                                ...
                                className={`ml-3 text-base ${isChecked ? "text-gray-400 line-through" : "text-white"} cursor-pointer`}
                              >
                                Episódio {episode}
                              </label>
                            </div>
                    ...
                    <div className="flex items-center p-2.5 bg-gray-800/40 rounded-md hover:bg-gray-700/60 transition-colors">
                      ...
                      <label
                        htmlFor="completed"
                        className={`ml-3 text-base ${progress?.completed ? "text-gray-400 line-through" : "text-white"} cursor-pointer`}
                      >
                ...
                <div className="bg-[#1c1c1c]/80 backdrop-blur-md rounded-xl shadow-2xl border border-gray-800/60">
                  ...
                  <ul className="space-y-2.5 max-h-60 overflow-y-auto custom-scrollbar-dark pr-1">
                    {participants.length > 0 ? (
                      participants.map((participant, index) => (
                        <li
                          key={index}
                          className="flex items-center text-sm text-gray-200 bg-gray-800/40 p-2.5 rounded-md shadow"
                        >
                          {participant.photoURL ? (
                            <img
                              src={participant.photoURL}
                              alt={participant.nickname}
                              className="w-7 h-7 rounded-full mr-2.5 object-cover border border-gray-600"
                            />
                          ) : (
                            <UserCircle
                              size={24}
                ...
                <div className="bg-[#1c1c1c]/80 backdrop-blur-md p-5 sm:p-6 rounded-xl shadow-2xl border border-gray-800/60">
                  ...
                  {totalProgress === 100 ? (
                    <div className="text-center">
                      ...
                      />
                      <p className="text-green-300 mb-3">Sessão concluída!</p>
                      <button
                        onClick={handleDeleteSession}
                        className="w-full px-5 py-2.5 bg-emerald-600 text-white font-semibold rounded-lg shadow-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-opacity-50 transition-colors flex items-center justify-center"
                      >
                    ...
                    <button
                      onClick={handleDeleteSession}
                      className="w-full px-5 py-2.5 bg-red-700/90 text-white font-semibold rounded-lg shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition-colors flex items-center justify-center"
                    >
