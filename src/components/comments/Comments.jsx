import { Loader2, MessageSquareText, Reply, Send, Trash2, UserCircle, XCircle } from "lucide-react";
...
const CommentItem = ({ comment, commentKey, onDeleteComment, onReply, currentUserId }) => {
  const canDelete = currentUserId === comment.userId;

  return (
    <div className="flex space-x-3 py-4 border-b border-gray-700/50 last:border-b-0">
      {comment.userPhotoURL ? (
        <img src={comment.userPhotoURL} alt={comment.userName} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover flex-shrink-0 mt-1" />
      ) : (
        <UserCircle size={36} className="text-gray-500 flex-shrink-0 mt-1 sm:w-10 sm:h-10 w-8 h-8" />
      )}
      <div className="flex-1">
        ...
        {comment.replyingToName && (
          <p className="text-xs text-gray-400 mb-1 italic">
            ↪ Respondendo a <span className="font-medium text-sky-500">{comment.replyingToName}</span>
          </p>
        )}
...
const Comments = ({ comments, onCommentSubmit, onDeleteComment }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (trimmedComment.length > MAX_COMMENT_CHARS) {
      toast.error(`Seu comentário excede o limite de ${MAX_COMMENT_CHARS} caracteres.`);
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      console.error("Erro na prop onCommentSubmit:", submitError);
  ...
  const getPlaceholderText = () => {
    ...
    const defaultNicknamePattern = user.uid ? `Usuário_${user.uid.substring(0, 5)}` : "";
    if (!nicknameFromContext || nicknameFromContext === defaultNicknamePattern) {
  ...
  const isTextareaDisabled = () => {
    ...
    const nicknameFromContext = user.nickname;
    const defaultNicknamePattern = user.uid ? `Usuário_${user.uid.substring(0, 5)}` : "";
    return !nicknameFromContext || nicknameFromContext === defaultNicknamePattern;
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 sm:p-8 bg-[#16161a] shadow-xl rounded-xl my-10 border border-gray-800/50">
      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6 flex items-center">
        <MessageSquareText size={28} className="mr-3 text-sky-500" />
      ...
      <form onSubmit={handleSubmit} className="mb-8">
        {replyingTo && (
          <div className="mb-3 p-2.5 bg-sky-900/30 border border-sky-700/50 rounded-md text-sm text-sky-300 flex justify-between items-center">
            <span>
              Respondendo a: <strong className="font-semibold">{replyingToName}< /strong>
            </span>
            <button
              type="button"
              onClick={cancelReply}
        ...
        <div className="relative">
          ...
          />
          <div className={`absolute bottom-2.5 right-3.5 text-xs font-medium
