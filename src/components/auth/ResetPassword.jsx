...
import { Mail, KeyRound, ArrowLeft, CheckCircle, AlertTriangle } from "lucide-react";
import { Message } from 'rsuite'; // Mantendo para consistência com Login, se desejar
...
const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      setMessage("Se o e-mail informado estiver cadastrado, você receberá um link para redefinir sua senha em instantes.");
      setTimeout(() => {
        navigate("/login");
      }, 4000);
    } catch (err) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-black px-4 text-gray-300">
      <div className="w-full max-w-md p-8 bg-[#1c1c1c] shadow-2xl rounded-xl border border-gray-800/70">
        ...
        {message && (
          <Message showIcon type="success" closable className="mb-4 !bg-green-600/20 !text-green-300 !border !border-green-500/50">
            {message}
          </Message>
        )}
        {error && (
          <Message showIcon type="error" closable className="mb-4 !bg-red-600/20 !text-red-300 !border !border-red-500/50">
            {error}
          </Message>
        )}
