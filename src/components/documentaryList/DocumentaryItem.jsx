...
const DocumentaryItem = ({ documentary, id }) => {
  ...
  const imageSrc =
    isHovered && documentary.gif ? documentary.gif : documentary.imageUrl;
    ...
    >
      ...
      >
        <figure>
          <img
            src={imageSrc}
            alt={`Capa do documentário ${documentary.name || 'Título do documentário'}`}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out rounded-lg flex items-end justify-center p-4">
          <div className="text-center text-white text-xs sm:text-sm max-h-22 overflow-hidden">
            <h3 className="font-bold text-xl">{documentary.name || "Título do documentário"}</h3>
            <p className="font-bold text-red-500">{documentary.rating || "Sem avaliação"}</p>
            <p className="text-gray-300">{documentary.category || "Categoria"}</p>
